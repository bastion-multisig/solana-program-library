...
use crate::{
    ...
    state::{
        ...
        proposal_transaction::{
            get_proposal_transaction_address_seeds, AccountMetaData, InstructionData,
            InstructionDataBrief, ProposalTransactionV2,
        },
...
pub fn process_insert_transaction_brief(
    program_id: &Pubkey,
    ...
    let instuctions_expanded = instructions
        .iter()
        .map(|ix| InstructionData {
            program_id: account_info_iter.next().unwrap().key.clone(),
            accounts: ix
                .accounts
                .iter()
                .map(|key| AccountMetaData {
                    pubkey: account_info_iter.next().unwrap().key.clone(),
    ...
    let proposal_transaction_data = ProposalTransactionV2 {
        ...
        hold_up_time,
        instructions: instuctions_expanded,
        executed_at: None,
